...
left join
  (
    SELECT user_key, arrival_key
          ...
          AND  arrival_created_at >= convert_timezone('America/New_York', 'UTC', '2020-07-01 00:00')
          GROUP BY 1,2
  ) jcr
...
left join
  (
    ...
    , su.loc_diff_group as loc_diff_group
    , count(distinct su.user_key) as user_count
    from
    #user_location_record su
    group by 1,2
  ) n
  on n.sent_date = i.sent_date and n.loc_diff_group = i.loc_diff_group
left join
  (
   select
  ...
  and n1.loc_diff_group = i.loc_diff_group
...
--      and c2.communication_sent_time >= convert_timezone('America/New_York', 'UTC', '2020-06-04 15:10')
     ...
     and coalesce(c2.outbound_communication_template_name,c2.communication_vendor_template_name) not in ('welcome_email','JC_TR_ALL_Welcome_Reg')
   group by 1,2,3
  ) n2
  on n2.sent_date = i.sent_date
  and n2.loc_diff_group = i.loc_diff_group
